...
public:
	/// \brief Constructor
	/// throws a ROMException if the file is not found or is not valid
    /// \param filepath path to a text file with the data to initialize the polyope
	/// the first line is a double value matching the chebyball radius of the polytope
	/// the line is an integer indicating the dimension of the polytope (ie the number of lines)
	/// the remaining lines describe the polytope. The first three doubles match a row of A, the last one is a row of b,
	/// with A(x) < b , and x is the angle variable
