...
IKSolver::~IKSolver()
{
    for(std::vector<PartialDerivativeConstraint*>::iterator it = constraints.begin();
...
bool IKSolver::StepClamping(planner::Node* limb, const Eigen::Vector3d& target, const Vector3d& direction, bool optimize ) const
{
    ...
    PartialDerivatives(limb, direction, postureVariation);
...
void IKSolver::PartialDerivative(planner::Node* limb, const Vector3d& direction, VectorXd& velocities, const int joint) const
{
    ...
    minDofs = dofs;
    minDofs(joint) = dofs[joint] - epsilon_;
    SetPose(limb, minDofs);
    Jacobian jacobMinus(limb);

    plusDofs = dofs;
    plusDofs(joint) = dofs[joint] + epsilon_;
	...
	{
		++ i;
        velocities(joint) += (*it)->Evaluate(limb, minDofs, plusDofs, i, jacobMinus, jacobPlus, epsilon_, direction);
	}
	if (i!= 0)
	{
...
void IKSolver::PartialDerivatives(planner::Node* limb, const Vector3d& direction, VectorXd& velocities) const
{
    for(int i =0; i< velocities.rows() ;++i)
	{
        PartialDerivative(limb, direction, velocities, i);
	}
}
...
void IKSolver::AddConstraint(Constraint constraint)
{
    switch(constraint)
    {
        case(FTR):
    {
        //constraints.push_back(new ForceManipulabilityConstraint());
    }
    case(AxisAlign):
    {
        //constraints.push_back(new VectorAlignmentConstraint());
    }
    default:
    {
        // NOTHING
