...
Eigen::MatrixXd ToEigen(const dd_MatrixPtr V)
{
...
void CheckEquilibrium(const Eigen::MatrixXd& contactTransforms, const Eigen::MatrixXd& graspTransforms, const Eigen::VectorXd& maxGraspingForces,  float friction, double flimit)
{
    ...
    Eigen::MatrixXd bcg(bg.rows() + bc.rows(), 1) ;
    Acg.block(0,0,Af.rows(),Af.cols()) = Af;
    Acg.block(Ag.rows(),Ag.cols(),Af.rows(),Af.cols()) = Ag;
    ...
    for(int i=1; i <= G->rowsize; i++)
    {
        if(G->matrix[i-1][0] == 0)
        {
            ...
            {
                V(realRowSize-1, j-1) = (double)(*(G->matrix[i-1][j-1]));
            }
            ++realRowSize;
        }
    }
    V = V.block(0,0,realRowSize, V.cols());
    Eigen::MatrixXd Pac(A.rows() + C.rows(), A.cols() + C.cols());
    Pac.block(0,0,A.rows(),A.cols()) = A;
    Pac.block(A.rows(),A.cols(),C.rows(),C.cols()) = C;
    Eigen::MatrixXd Vp = V * Pac.transpose();


    //now reconvert to H rep
    dd_FreeMatrix(G);dd_FreeMatrix(matrix);dd_FreePolyhedra(poly);
