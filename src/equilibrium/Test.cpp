                      ...
                      const float mass, float friction, double flimit)
{
    ...
    if(nbContacts > 0)
    {
        ...
        {
            C.block<3,3>(0,3*i) = skew(contactTransforms[i].block<3,1>(0, 2));
    ...
    if(nbGrasps > 0)
    {
        ...
        {
            C.block<3,3>(0,3*(i+nbContacts)) = skew(graspTransforms[i].block<3,1>(i, 2));
            Ag(6*i  ,3*i)   = 1;
            Ag(6*i+1,3*i)   =-1;
            Ag(6*i+2,3*i+1) = 1;
            Ag(6*i+3,3*i+1) =-1;
    ...
    if(nbGrasps>0)
    {
        ...
        bcg.block(afCols,0,agRows,1) = bg;
    }
